...
test("refresh/recalculate action is expected in non-loading states", () => {
  assert.equal(
    resolveSubscriptionsCardState({ loading: false, error: null, summary: { candidates: [] } }),
    "empty",
  );
  assert.equal(
    resolveSubscriptionsCardState({
      loading: false,
      error: null,
      summary: { candidates: [{ merchant_display: "Netflix" }] },
    }),
    "ready",
  );
});
...
test("loading and error states still resolve correctly", () => {
  assert.equal(
    resolveSubscriptionsCardState({ loading: true, error: null, summary: null }),
    "loading",
  );
  assert.equal(
    resolveSubscriptionsCardState({ loading: false, error: "boom", summary: null }),
    "error",
  );
});
