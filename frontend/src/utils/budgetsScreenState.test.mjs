...
test("budget modal helpers build deterministic create/edit states", () => {
  ...
  assert.deepEqual(editBudgetModalState({ budget_id: "b1", category_id: "c1", amount: 125.5 }), {
    editingBudgetId: "b1",
    isCategoryBudgetForm: true,
    selectedCategory: "c1",
    budgetAmount: "125.5",
...
test("budget view state resolves loading/no-profile/ready and modal title transitions", () => {
  ...
  assert.equal(
    deriveBudgetsViewState({ activeProfile: { profile_id: "p1" }, isLoading: true }),
    "loading",
  );
  assert.equal(
    deriveBudgetsViewState({ activeProfile: { profile_id: "p1" }, isLoading: false }),
    "ready",
  );

  assert.equal(
    deriveBudgetModalTitle({ editingBudgetId: null, isCategoryBudgetForm: false }),
    "Set Total Budget",
  );
  assert.equal(
    deriveBudgetModalTitle({ editingBudgetId: null, isCategoryBudgetForm: true }),
    "Add Category Budget",
  );
  assert.equal(
    deriveBudgetModalTitle({ editingBudgetId: "b1", isCategoryBudgetForm: true }),
    "Edit Budget",
  );
});
...
test("budget CRUD orchestration helpers keep create/edit/delete flows deterministic", () => {
  ...
  assert.deepEqual(
    afterDeleteCategory.budgets.map((budget) => budget.budget_id),
    ["cat-2"],
  );
