...
test("chat modal send/disable guard supports pending->success/failure orchestration preconditions", () => {
  assert.equal(
    deriveIsChatSubmitDisabled({ profileId: "", isLoading: false, prompt: "hello world" }),
    true,
  );
  assert.equal(
    deriveIsChatSubmitDisabled({ profileId: "   ", isLoading: false, prompt: "hello world" }),
  ...
  assert.equal(
    deriveIsChatSubmitDisabled({ profileId: "p1", isLoading: true, prompt: "hello world" }),
    true,
  );
  assert.equal(
    deriveIsChatSubmitDisabled({ profileId: "p1", isLoading: false, prompt: "ok" }),
    true,
  );
  assert.equal(
    deriveIsChatSubmitDisabled({ profileId: "p1", isLoading: false, prompt: "How to save more?" }),
    false,
  );
  assert.equal(
    deriveIsChatSubmitDisabled({ profileId: "p1", isLoading: false, prompt: "x".repeat(301) }),
    true,
  );
  ...
  const blocked = buildChatSendQuestionParams({
    profileId: "p1",
    isLoading: true,
  ...
  const tooLongBlocked = buildChatSendQuestionParams({
    profileId: "p1",
    isLoading: false,
    prompt: "x".repeat(301),
  });
  ...
  const blankProfileBlocked = buildChatSendQuestionParams({
    profileId: "   ",
    isLoading: false,
    prompt: "valid question",
  });
...
test("chat modal close/reopen handling keeps retry-capable state and profile isolation safety", () => {
  const interrupted = deriveChatSessionOnModalClose({
    ...
    session: {
      prompt: "draft",
      messages: [
        { id: "a1", role: "assistant", status: "pending", text: "typing...", retryQuestion: "Q1" },
      ],
    },
  ...
  const pendingEvenWhenNotLoading = deriveChatSessionOnModalClose({
    ...
    session: {
      prompt: "draft",
      messages: [
        { id: "a2", role: "assistant", status: "pending", text: "typing...", retryQuestion: "Q2" },
      ],
    },
...
test("failed-message retry affordance is disabled deterministically when loading or retryQuestion is missing", () => {
  assert.equal(
    deriveCanRetryFailedMessage({
      message: { status: "failed", retryQuestion: "How can I save?" },
      isLoading: false,
    }),
    true,
  );
  assert.equal(
    deriveCanRetryFailedMessage({
      message: { status: "failed", retryQuestion: "  " },
      isLoading: false,
    }),
    false,
  );
  assert.equal(
    deriveCanRetryFailedMessage({
      message: { status: "sent", retryQuestion: "How can I save?" },
      isLoading: false,
    }),
    false,
  );
...
test("chat suggestion affordance is disabled deterministically when loading or suggestion text is invalid", () => {
  assert.equal(
    deriveCanApplyChatSuggestion({ isLoading: false, suggestion: "How can I save more?" }),
    true,
  );
  assert.equal(
    deriveCanApplyChatSuggestion({ isLoading: true, suggestion: "How can I save more?" }),
    false,
  );
  assert.equal(deriveCanApplyChatSuggestion({ isLoading: false, suggestion: "  " }), false);
  assert.equal(
    deriveCanApplyChatSuggestion({ isLoading: false, suggestion: "x".repeat(301) }),
    false,
  );
});
