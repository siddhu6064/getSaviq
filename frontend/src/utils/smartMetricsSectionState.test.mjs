...
test("smart metrics section state resolves loading, error, empty, success", () => {
  ...
  assert.equal(
    deriveSmartMetricsViewState({ isLoading: true, error: "", cards: emptyCards }),
    "loading",
  );
  assert.equal(
    deriveSmartMetricsViewState({ isLoading: false, error: "failed", cards: successCards }),
    "error",
  );
  assert.equal(
  ...
  assert.equal(
    deriveSmartMetricsViewState({ isLoading: false, error: "", cards: emptyCards }),
    "empty",
  );
  assert.equal(
    deriveSmartMetricsViewState({ isLoading: false, error: "", cards: successCards }),
    "success",
  );
});
