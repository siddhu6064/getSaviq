...
test("canSubmitTransaction prevents duplicate submit and invalid input", () => {
  assert.equal(
    canSubmitTransaction({ isSubmitting: true, amount: "12.50", hasActiveProfile: true }),
    false,
  );
  assert.equal(
    canSubmitTransaction({ isSubmitting: false, amount: "", hasActiveProfile: true }),
    false,
  );
  assert.equal(
    canSubmitTransaction({ isSubmitting: false, amount: "0", hasActiveProfile: true }),
    false,
  );
  assert.equal(
    canSubmitTransaction({ isSubmitting: false, amount: "12.50", hasActiveProfile: false }),
    false,
  );
  assert.equal(
    canSubmitTransaction({ isSubmitting: false, amount: "12.50", hasActiveProfile: true }),
    true,
  );
});
