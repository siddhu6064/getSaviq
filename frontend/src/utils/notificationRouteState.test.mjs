...
test("notification route mapping keeps add/stats/home assumptions deterministic", () => {
  ...
  assert.equal(deriveRouteFromNotificationData({ screen: "add" }), "/(tabs)/add");
  ...
  assert.equal(deriveRouteFromNotificationData({ screen: " Stats " }), "/(tabs)/stats");
  assert.equal(
    deriveRouteFromNotificationData({ screen: "ADD", amount: " 10.00 " }),
    "/(tabs)/add?amount=10.00&fromShortcut=true",
  );
  assert.equal(
