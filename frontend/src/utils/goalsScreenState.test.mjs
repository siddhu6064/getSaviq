...
test("goals view state resolves no-profile/loading/error/empty/ready states", () => {
  assert.equal(
    deriveGoalsViewState({ activeProfile: null, isLoading: false, error: "", goalsCount: 0 }),
    "no_profile",
  );
  assert.equal(
    deriveGoalsViewState({
      activeProfile: { profile_id: "p" },
      isLoading: true,
      error: "",
      goalsCount: 0,
    }),
...
test("goals create/edit/delete orchestration helpers remain deterministic", () => {
  ...
  assert.deepEqual(
    remaining.map((goal) => goal.goal_id),
    ["g2"],
  );
});
