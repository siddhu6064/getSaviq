...
async function installMockApi(page: Page) {
  ...
  const buildBudgetProgress = (profileId: string) => {
    ...
    const total =
      state.budgets.find(
        (budget) =>
          budget.profile_id === profileId && normalizeCategoryId(budget.category_id) === null,
      ) || null;
  ...
  const logSmartMetricsDiag = (
    ...
    console.log(
      `[smart-metrics-diag] ${tag} method=${method} url=${payload.url} path=${pathname} search=${search}${status}${mode}${action}`,
    );
  };
  ...
  const findLastTrackedRequest = (url: string, method: string) =>
    [...state.smartMetricsIntercept.requests]
      .reverse()
      .find(
        (entry) => entry.url === url && entry.method === method && !entry.finished && !entry.failed,
      );
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (path === "/api/stats/summary" && method === "GET") {
      ...
      const byCategoryMap = new Map<
        string,
        { amount: number; category_id: string; name: string; color: string; icon: string }
      >();
      ...
      if (state.insightsV2Mode === "empty" || state.insightsV2Mode === "no_budget") {
        ...
        return json({
          ...
          monthly: {
            ...
            budget_risk: {
              period_type: "monthly",
              status: state.insightsV2Mode === "no_budget" ? "no_budget" : "ok",
              budget_amount: 0,
              current_spend: 0,
              progress_percent: 0,
              remaining_days: 0,
  ...
  return {
    ...
    setInsightsV2Mode: (
      mode: "normal" | "empty" | "error" | "no_budget" | "no_spike" | "category_trend",
    ) => {
      state.insightsV2Mode = mode;
    },
    setSmartMetricsMode: (mode: "normal" | "loading" | "empty" | "error") => {
      state.smartMetricsMode = mode;
    },
    setAiChatMode: (mode: "success" | "error") => {
      state.aiChatMode = mode;
    },
    setSubscriptionsMode: (mode: "ready" | "loading" | "empty" | "error") => {
      state.subscriptionsMode = mode;
    },
    setWeeklyDigestMode: (mode: "success" | "loading" | "empty" | "error") => {
      state.weeklyDigestMode = mode;
    },
    setSavingsGoalsMode: (mode: "success" | "loading" | "empty" | "error") => {
      state.savingsGoalsMode = mode;
    },
  };
}
...
async function logSmartMetricsDomDebug(page: Page, label: string) {
  const [stage, mode, requestState] = await Promise.all([
    page
      .getByTestId("smart-metrics-debug-last-stage")
      .textContent()
      .catch(() => null),
    page
      .getByTestId("smart-metrics-mode")
      .textContent()
      .catch(() => null),
    page
      .getByTestId("smart-metrics-request-state")
      .textContent()
      .catch(() => null),
  ]);
  console.log(
    `[smart-metrics-dom] ${label} stage=${stage || ""} mode=${mode || ""} requestState=${requestState || ""}`,
  );
}

function assertDeterministicSmartMetricsIntercept(
  intercept: {
    requests: Array<{
      url: string;
      method: string;
  ...
  expect(
    ...
    `smart metrics request count/urls: ${summaryText}`,
  ).toBeGreaterThan(0);
  expect(intercept.hitCount, `smart metrics intercept hits: ${summaryText}`).toBe(
    intercept.requests.length,
  );
  expect(intercept.fulfilledCount, `smart metrics fulfilled count: ${summaryText}`).toBe(
    intercept.requests.length,
  );
  expect(
    intercept.requests.every((request) => request.method === "GET"),
    `smart metrics method mismatch: ${summaryText}`,
  ).toBeTruthy();
  expect(
    intercept.requests.every((request) => request.matched),
    `smart metrics route-match mismatch: ${summaryText}`,
  ).toBeTruthy();
  expect(
    intercept.requests.every((request) => request.action === "fulfilled"),
    `smart metrics route-action mismatch: ${summaryText}`,
  ).toBeTruthy();
  expect(
    intercept.requests.every((request) => /\/dashboard\/metrics(\?|$)/.test(request.url)),
    `smart metrics request URL mismatch: ${summaryText}`,
  ).toBeTruthy();
  expect(
    intercept.requests.some((request) => request.url === requestUrlFromDom),
    `smart metrics request URL marker mismatch: ${summaryText}`,
  ).toBeTruthy();
  expect(intercept.hangingRequests, `smart metrics hanging requests: ${summaryText}`).toHaveLength(
    0,
  );
}
...
test.describe("e2e smoke", () => {
  ...
  test("budget happy path creates and renders progress summary", async ({ page }) => {
    ...
    const refreshBudgetProgressResponse = page.waitForResponse(
      (resp) => resp.url().includes("/api/budgets/progress") && resp.request().method() === "GET",
    );
  ...
  test("dashboard Smart Insights shows retry on failure", async ({ page }) => {
    ...
    await expect(page.getByTestId("smart-insights-error-diagnostics")).toContainText(
      "Ref: request:500:SERVER_ERROR",
    );
  ...
  test("dashboard Smart Insights falls back cleanly when no overspending alert exists", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
  ...
  test("dashboard Smart Insights renders category trend insight when selected signal", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
  ...
  test("dashboard Smart Dashboard Net Balance card renders deterministic success state", async ({
    page,
  }) => {
    await installMockApi(page);
  ...
  test("dashboard Smart Dashboard Net Balance card renders deterministic error state when summary request fails", async ({
    page,
  }) => {
    await installMockApi(page);
    ...
    await expect(page.getByTestId("smart-dashboard-net-balance-error")).toHaveText(
      "Net balance is unavailable right now.",
    );
    await expect(page.getByTestId("smart-dashboard-net-balance-value")).toHaveCount(0);
  });

  test("dashboard Smart Dashboard Net Balance card renders deterministic loading state", async ({
    page,
  }) => {
    await installMockApi(page);
  ...
  test("dashboard Smart Dashboard Net Balance card renders deterministic empty state when summary has no net balance", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
    ...
    await expect(page.getByTestId("smart-dashboard-net-balance-empty")).toHaveText(
      "No balance data yet.",
    );
    await expect(page.getByTestId("smart-dashboard-net-balance-value")).toHaveCount(0);
  });

  test("dashboard Smart Dashboard Net Balance error state coexists deterministically with Smart Insights visibility", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Net Balance remains deterministic after dashboard re-entry in same session", async ({
    page,
  }) => {
    await installMockApi(page);
    await registerAndOpenDashboard(page);
  ...
  test("dashboard Smart Dashboard Net Balance reloads deterministically from success state", async ({
    page,
  }) => {
    await installMockApi(page);
    await registerAndOpenDashboard(page);
  ...
  test("dashboard Smart Dashboard Net Balance switches deterministically from populated profile to empty profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Net Balance switches deterministically from populated profile to error profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Net Balance recovers deterministically from error profile to populated profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Net Balance recovers deterministically from empty profile to populated profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Income card renders deterministic loading state", async ({
    page,
  }) => {
    await installMockApi(page);
    let releaseSummaryResponse: (() => void) | null = null;
  ...
  test("dashboard Smart Dashboard Income card renders deterministic error state when summary request fails", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
    ...
    await expect(page.getByTestId("smart-dashboard-income-error")).toHaveText(
      "Income is unavailable right now.",
    );
    await expect(page.getByTestId("smart-dashboard-income-value")).toHaveCount(0);
  });

  test("dashboard Smart Dashboard Income card renders deterministic empty state when summary has no income", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
    ...
    await expect(page.getByTestId("smart-dashboard-income-empty")).toHaveText(
      "No income data yet.",
    );
    await expect(page.getByTestId("smart-dashboard-income-value")).toHaveCount(0);
  });

  test("dashboard Smart Dashboard Income card renders deterministic kickoff success state", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Income error state coexists deterministically with Net Balance visibility", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Income empty state coexists deterministically with Net Balance success visibility", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Income reloads deterministically from success state", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Income switches deterministically from populated profile to populated profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Income switches deterministically from populated profile to empty profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Income switches deterministically from populated profile to error profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Income switches deterministically from error profile to populated profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Income switches deterministically from empty profile to populated profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Total Spend card renders deterministic loading state", async ({
    page,
  }) => {
    await installMockApi(page);
    let releaseSummaryResponse: (() => void) | null = null;
  ...
  test("dashboard Smart Dashboard Total Spend card renders deterministic error state when summary request fails", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
    ...
    await expect(page.getByTestId("smart-dashboard-total-spend-error")).toHaveText(
      "Total spend is unavailable right now.",
    );
    await expect(page.getByTestId("smart-dashboard-total-spend-value")).toHaveCount(0);
  });

  test("dashboard Smart Dashboard Total Spend card renders deterministic empty state when summary has no total spend", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
    ...
    await expect(page.getByTestId("smart-dashboard-total-spend-empty")).toHaveText(
      "No spend data yet.",
    );
    await expect(page.getByTestId("smart-dashboard-total-spend-value")).toHaveCount(0);
  });

  test("dashboard Smart Dashboard Total Spend card renders deterministic kickoff success state", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Total Spend error state coexists deterministically with Net Balance and Income visibility", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Total Spend empty state coexists deterministically with Net Balance and Income success visibility", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Total Spend reloads deterministically from success state", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Total Spend switches deterministically from populated profile to populated profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Total Spend switches deterministically from populated profile to empty profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Total Spend switches deterministically from populated profile to error profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Total Spend switches deterministically from error profile to populated profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Total Spend switches deterministically from empty profile to populated profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Current Month Spend card renders deterministic loading state", async ({
    page,
  }) => {
    await installMockApi(page);
    let releaseSummaryResponse: (() => void) | null = null;
  ...
  test("dashboard Smart Dashboard Current Month Spend card renders deterministic error state when summary request fails", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
    ...
    await expect(page.getByTestId("smart-dashboard-current-month-spend-error")).toHaveText(
      "Current month spend is unavailable right now.",
    );
    await expect(page.getByTestId("smart-dashboard-current-month-spend-value")).toHaveCount(0);
  });

  test("dashboard Smart Dashboard Current Month Spend card renders deterministic empty state when summary has no current month spend", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
    ...
    await expect(page.getByTestId("smart-dashboard-current-month-spend-empty")).toHaveText(
      "No current month spend data yet.",
    );
    await expect(page.getByTestId("smart-dashboard-current-month-spend-value")).toHaveCount(0);
  });

  test("dashboard Smart Dashboard Current Month Spend card renders deterministic kickoff success state", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
    ...
    await expect(page.getByTestId("smart-dashboard-current-month-spend-value")).toBeVisible();
    await expect(page.getByTestId("smart-dashboard-current-month-spend-value")).toHaveText(
      /321\.09/,
    );
  });

  test("dashboard Smart Dashboard Current Month Spend error state coexists deterministically with Net Balance, Income, and Total Spend visibility", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Current Month Spend empty state coexists deterministically with Net Balance, Income, and Total Spend success visibility", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Current Month Spend reloads deterministically from success state", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Current Month Spend switches deterministically from populated profile to populated profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Current Month Spend switches deterministically from populated profile to empty profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Current Month Spend switches deterministically from populated profile to error profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Current Month Spend switches deterministically from error profile to populated profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Current Month Spend switches deterministically from empty profile to populated profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard MoM Change card renders deterministic loading state", async ({
    page,
  }) => {
    await installMockApi(page);
    let releaseSummaryResponse: (() => void) | null = null;
  ...
  test("dashboard Smart Dashboard MoM Change card renders deterministic error state when summary request fails", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
    ...
    await expect(page.getByTestId("smart-dashboard-mom-change-error")).toHaveText(
      "MoM change is unavailable right now.",
    );
    await expect(page.getByTestId("smart-dashboard-mom-change-value")).toHaveCount(0);
  });

  test("dashboard Smart Dashboard MoM Change card renders deterministic empty state when summary has no MoM change", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
    ...
    await expect(page.getByTestId("smart-dashboard-mom-change-empty")).toHaveText(
      "No month-over-month trend yet.",
    );
    await expect(page.getByTestId("smart-dashboard-mom-change-value")).toHaveCount(0);
  });

  test("dashboard Smart Dashboard MoM Change card renders deterministic kickoff success state", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard MoM Change error state coexists deterministically with Net Balance visibility", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard MoM Change empty state coexists deterministically with Income success visibility", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard MoM Change empty state keeps Total Spend and Current Month Spend explicitly visible and stable", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard MoM Change reloads deterministically from success state", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard MoM Change switches deterministically from populated profile to populated profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard MoM Change switches deterministically from populated profile to empty profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard MoM Change switches deterministically from populated profile to error profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard MoM Change switches deterministically from error profile to populated profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard MoM Change switches deterministically from empty profile to populated profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/analytics/summary**", async (route) => {
  ...
  test("dashboard Smart Dashboard Top Savings Goal card renders deterministic loading state", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    ...
    await expect(
      page.getByTestId("smart-dashboard-top-savings-goal-loading-secondary"),
    ).toBeVisible();
  });

  test("dashboard Smart Dashboard Top Savings Goal card renders deterministic error state when savings goals request fails", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    ...
    await expect(page.getByTestId("smart-dashboard-top-savings-goal-error")).toHaveText(
      "Savings goal is unavailable right now.",
    );
    await expect(page.getByTestId("smart-dashboard-top-savings-goal-error-secondary")).toHaveText(
      "Savings goal is unavailable right now.",
    );
  });

  test("dashboard Smart Dashboard Top Savings Goal card renders deterministic empty state without active goals", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    ...
    await expect(page.getByTestId("smart-dashboard-top-savings-goal-empty")).toHaveText(
      "No active savings goals yet.",
    );
    await expect(page.getByTestId("smart-dashboard-top-savings-goal-empty-secondary")).toHaveText(
      "No active savings goals yet.",
    );
  });

  test("dashboard Smart Dashboard Top Savings Goal card renders deterministic kickoff success state", async ({
    page,
  }) => {
    await installMockApi(page);
    await registerAndOpenDashboard(page);
  ...
  test("dashboard Smart Dashboard Top Savings Goal error state coexists deterministically with Net Balance visibility", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setSavingsGoalsMode("error");
  ...
  test("dashboard Smart Dashboard Top Savings Goal empty state coexists deterministically with Income success visibility", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setSavingsGoalsMode("empty");
  ...
  test("dashboard Smart Dashboard Top Savings Goal empty state keeps Total Spend, Current Month Spend, and MoM Change explicitly visible and stable", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setSavingsGoalsMode("empty");
  ...
  test("dashboard Smart Dashboard Top Savings Goal reloads deterministically from success state", async ({
    page,
  }) => {
    await installMockApi(page);
    let requestCount = 0;
    ...
    await expect(page.getByTestId("smart-dashboard-top-savings-goal-title")).not.toHaveText(
      "Vacation Fund",
    );
  ...
  test("dashboard Smart Dashboard Top Savings Goal switches deterministically from populated profile to populated profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/savings-goals**", async (route) => {
  ...
  test("dashboard Smart Dashboard Top Savings Goal switches deterministically from populated profile to empty profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/savings-goals**", async (route) => {
  ...
  test("dashboard Smart Dashboard Top Savings Goal switches deterministically from populated profile to error profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/savings-goals**", async (route) => {
  ...
  test("dashboard Smart Dashboard Top Savings Goal switches deterministically from error profile to populated profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/savings-goals**", async (route) => {
  ...
  test("dashboard Smart Dashboard Top Savings Goal switches deterministically from empty profile to populated profile", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/savings-goals**", async (route) => {
  ...
  test("dashboard Subscriptions manual recalc reloads deterministically from ready state", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
  ...
  test("dashboard Subscriptions remains stable after dashboard re-entry in same session", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setSubscriptionsMode("ready");
  ...
  test("dashboard Subscriptions refreshes deterministically when profile context changes", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setSubscriptionsMode("ready");
  ...
  test("dashboard Subscriptions switches deterministically between populated profiles without stale content", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setSubscriptionsMode("ready");
  ...
  test("dashboard Subscriptions remains deterministic across rapid populated profile switch and switch-back", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setSubscriptionsMode("ready");
  ...
  test("dashboard Subscriptions profile refresh coexists deterministically with Smart Insights visibility", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setSubscriptionsMode("ready");
  ...
  test("dashboard Subscriptions switches deterministically from populated profile to empty profile", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setSubscriptionsMode("ready");
    ...
    await expect(page.getByTestId("subscriptions-empty-state")).toContainText(
  ...
  test("dashboard Subscriptions switches deterministically from empty profile back to populated profile", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setSubscriptionsMode("ready");
  ...
  test("dashboard Subscriptions card renders deterministic error state without blocking dashboard usage", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
  ...
  test("dashboard Subscriptions card recovers from error after retry in same session", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setSubscriptionsMode("error");
  ...
  test("dashboard Subscriptions error and recovery coexist safely with Smart Insights card", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setSubscriptionsMode("error");
  ...
  test("dashboard Subscriptions card transitions from loading to ready deterministically", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setSubscriptionsMode("loading");
  ...
  test("dashboard Weekly Digest renders deterministic empty terminal state", async ({ page }) => {
    ...
    await expect(page.getByTestId("weekly-digest-card-empty")).toContainText(
  ...
  test("dashboard Weekly Digest renders deterministic success summary state", async ({ page }) => {
    ...
    await expect(page.getByTestId("weekly-digest-supporting-line")).toHaveText(
      "Income $500.00 • Expense $120.00",
    );
  });
  ...
  test("dashboard Weekly Digest renders deterministic loading state", async ({ page }) => {
    ...
    await expect(page.getByTestId("weekly-digest-card-loading-message")).toHaveText(
  ...
  test("dashboard Weekly Digest renders deterministic error terminal state", async ({ page }) => {
    ...
    await expect(page.getByTestId("weekly-digest-card-error-message")).toHaveText(
      "Weekly digest is unavailable right now.",
    );
  });

  test("dashboard Weekly Digest manual refresh reloads deterministically from success state", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setWeeklyDigestMode("success");
  ...
  test("dashboard Weekly Digest error state coexists deterministically with Smart Insights visibility", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setWeeklyDigestMode("error");
  ...
  test("dashboard Weekly Digest remains deterministic after dashboard re-entry in same session", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setWeeklyDigestMode("success");
  ...
  test("dashboard Weekly Digest refreshes deterministically when profile context changes", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setWeeklyDigestMode("success");
    ...
    await expect(page.getByTestId("weekly-digest-supporting-line")).toHaveText(
      "Income $920.00 • Expense $410.00",
    );
    await expect(page.getByText("Income $500.00 • Expense $120.00")).toHaveCount(0);
  });

  test("dashboard Weekly Digest switches deterministically from populated profile to empty profile", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setWeeklyDigestMode("success");
  ...
  test("dashboard Weekly Digest switches deterministically from empty profile back to populated profile", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setWeeklyDigestMode("success");
  ...
  test("dashboard Weekly Digest switches deterministically between populated profiles without stale content", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setWeeklyDigestMode("success");
  ...
  test("dashboard Weekly Digest remains deterministic across rapid populated profile switch and switch-back", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setWeeklyDigestMode("success");
  ...
  test("dashboard AI Chat modal supports deterministic whitespace-disabled guard and close path", async ({
    page,
  }) => {
    await installMockApi(page);
    await registerAndOpenDashboard(page);
  ...
  test("dashboard AI Chat modal supports deterministic successful submit path", async ({
    page,
  }) => {
    await installMockApi(page);
    await registerAndOpenDashboard(page);
    ...
    await page
      .getByTestId("ai-chat-question-input")
      .fill("How can I reduce dining spend this week?");
    await page.getByTestId("ai-chat-submit").click();

    await expect(page.getByTestId("ai-chat-submit")).toBeDisabled();

    await expect(page.getByTestId("ai-chat-latest-response")).toBeVisible();
    await expect(page.getByTestId("ai-chat-latest-response")).toContainText(
      "Focus on dining this week",
    );
  ...
  test("dashboard AI Chat modal supports deterministic failed submit path", async ({ page }) => {
    ...
    await expect(page.getByTestId("ai-chat-error")).toContainText(
  ...
  test("dashboard AI Chat modal supports deterministic retry after failure in same session", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setAiChatMode("error");
  ...
  test("dashboard AI Chat modal resets latest response after close and reopen", async ({
    page,
  }) => {
    await installMockApi(page);
    await registerAndOpenDashboard(page);
    ...
    await expect(page.getByTestId("ai-chat-latest-response")).toHaveCount(0);
    await expect(page.getByTestId("ai-chat-error")).toHaveCount(0);
    await expect(page.getByTestId("ai-chat-question-input")).toHaveValue(
  ...
  test("dashboard Smart Metrics renders six cards with valid payload", async ({ page }) => {
    ...
    const requestUrlFromDom =
      (await page.getByTestId("smart-metrics-request-url").textContent()) || "";
  ...
  test("dashboard Smart Metrics error state is isolated and keeps dashboard usable", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    ...
    assertDeterministicSmartMetricsIntercept(intercept, requestUrlFromDom);
    expect(
      intercept.requests.some((request) => request.status !== null && request.status >= 500),
    ).toBeTruthy();
  });

  test("dashboard Smart Metrics helper affordances render for applicable metrics", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setSmartMetricsMode("normal");
  ...
  test("dashboard V2 release sanity keeps cross-surface coexistence stable during digest error and savings-goal empty states", async ({
    page,
  }) => {
    const mockApi = await installMockApi(page);
    mockApi.setWeeklyDigestMode("error");
  ...
  test("dashboard V2 release sanity profile switch keeps key completed surfaces in stable terminal states", async ({
    page,
  }) => {
    await installMockApi(page);
    await registerAndOpenDashboard(page);
  ...
  test("release candidate core sanity register/login lands on stable app shell", async ({
    page,
  }) => {
    await installMockApi(page);
    await registerAndOpenDashboard(page);
  ...
  test("release candidate core sanity fresh-account add-first-transaction happy path stays stable", async ({
    page,
  }) => {
    await installMockApi(page);
    await registerAndOpenDashboard(page);
  ...
  test("release candidate core sanity settings delete-account failure path remains stable with friendly message", async ({
    page,
  }) => {
    await installMockApi(page);
    await page.route("**/api/auth/account**", async (route) => {
  ...
  test("launch signoff app-shell navigation sanity dashboard to transactions to dashboard remains stable", async ({
    page,
  }) => {
    await installMockApi(page);
    await registerAndOpenDashboard(page);
  ...
  test("launch signoff app-shell navigation sanity dashboard to settings to dashboard remains stable", async ({
    page,
  }) => {
    await installMockApi(page);
    await registerAndOpenDashboard(page);
  ...
  test("launch signoff app-shell navigation sanity dashboard to analytics to dashboard remains stable", async ({
    page,
  }) => {
    await installMockApi(page);
    await registerAndOpenDashboard(page);
  ...
  test("final release signoff confirmation dashboard shell opens with core V2 card visible", async ({
    page,
  }) => {
    await installMockApi(page);
    await registerAndOpenDashboard(page);
  ...
  test("final release signoff confirmation app-shell reaches transactions route deterministically", async ({
    page,
  }) => {
    await installMockApi(page);
    await registerAndOpenDashboard(page);
  ...
  test("final release signoff confirmation return to dashboard remains stable after analytics route", async ({
    page,
  }) => {
    await installMockApi(page);
    await registerAndOpenDashboard(page);
